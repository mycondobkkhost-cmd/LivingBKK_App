-- สถานะทรัพย์ตอนลงประกาศ (ว่าง / รีโนเวท / มีผู้เช่า / ขายพร้อมผู้เช่า)

ALTER TABLE public.listings
  ADD COLUMN IF NOT EXISTS occupancy_status text
    CHECK (
      occupancy_status IS NULL
      OR occupancy_status IN ('ready', 'renovating', 'tenanted', 'sale_with_tenant')
    ),
  ADD COLUMN IF NOT EXISTS viewing_allowed_during boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.listings.occupancy_status IS
  'ready | renovating | tenanted | sale_with_tenant';
COMMENT ON COLUMN public.listings.viewing_allowed_during IS
  'นัดดูระหว่างรีโนเวทหรือมีผู้เช่าอยู่ได้';
